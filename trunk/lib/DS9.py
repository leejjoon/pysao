...
## 2006-08-25: improved import routine for "ImageBaseHDU" and, etc.
## 2006-08-25: definition of  _pyfits2string2 removed


########## TODO
### ds9 should be initialized with currently running ds9, e.g., give xpa name
...
def _load_logo():
    ...
    import binascii
    _ds9_python_logo = binascii.a2b_hqx(_ds9_python_logo_text)[0]
...
class ds9(object):
    ...
    def __init__(self, path="ds9"):
        ...
        if displaydev:
            ...
            self.numdisp_dev = ""
            self.numdisp = None
        #self.numdisp = numdisplay.NumDisplay()
        #self.numdisp.open("unix:%s" % self.ds9_unix_name)

        self.xpa.set("fits", _ds9_python_logo)
        self._zoom_per_degree = None
        self.wcs_frame = []
    ...
    def run_unixonly_ds9(self):
        """ start ds9 w/ unique title """
    
        try:
            old_xpa = Set(xpa.get("xpans").rstrip().split("\n"))
        except:
            # when xpans is not ruuning
            old_xpa = Set([])
        ...
        for i in range(99):
            instance_numer += 1
            n = pyds9_name(instance_numer)
            if [s for s in list(old_xpa) if n in s]:
                continue
            else:
                break

        _name = pyds9_name(instance_numer)

        from tempfile import mkdtemp
        self._tmpd_name = mkdtemp()
        unix_name = "%s/.IMT" % self._tmpd_name

        os.system(self.path + " -title %s -unix_only -unix %s &" \
                  % (_name, unix_name))
        

        for i in range(200):
            try:
                new_xpa = Set(xpa.get("xpans").rstrip().split("\n")) - old_xpa
                new_instance = [s for s in list(new_xpa) if _name in s]
                r = new_instance.pop()
                break
            except:
                time.sleep(0.5)
        else:
            raise "DS9 error : timed out while running new instance of ds9"

        xpa_name = r.split()[3]

        return xpa_name, unix_name

    def run_unixonly_ds9_v2(self):
        """ start ds9 w/ unique title """
        ...
        self._tmpd_name = mkdtemp(prefix=env.get("USER",""),
                                  dir="/tmp/.xpa")
    ...
    def view_fits(self, hdu, zscale=None):
        try:
        ...
        if _pyfits2string:
            ...
            raise "no fits support available"
    ...
    def view_array(self, img, header=None, arch=""):
        ...
        try:
            ...
            raise "unsupported array shape : %s" % repr(img.shape)
        ...
        if 0: # numarray
            ...
            from numarray import Bool,Int8, Int16, Int32, Int64,\
                 Float32, Float64
            ...
            if t in [Float32, Float64]:
                bitpix = t.bytes * -8
            elif t in [Int8, Int16, Int32]: # Int64 not supported?
                bitpix = t.bytes * 8
            elif t in [Bool]:
                img = img.astype(Int8)
                bitpix = 8
    ...
    def mark(self, x, y, size, coord="fk5"):
        group_name = "_ds9_jj123t1"
    ...
    def set_zoom_per_degree(self, zoom, degree):
        self._zoom_per_degree = (1./zoom) / degree

    def zoom2degree(self, degree):
        if self._zoom_per_degree:
            self.set("zoom to %e" % (1./(self._zoom_per_degree*degree)))
